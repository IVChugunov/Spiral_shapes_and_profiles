...
def fit_profile_2d(col_arr, phi_arr, r_arr, flip, d_cat):
    ...
    for i in range(len(col_arr)):
        col = col_arr[i]
        fn = f"../../images/grids/2d_fits_final_masked/{this_gal}_{col}.png"
        ...
        arm_str = fits.getdata(f'str_arms_azavg_masked/arm_str_{col}.fits')
        sigma_str = fits.getdata(f'str_arms_sigma_masked/arm_str_{col}.fits')
        ...
        fig.tight_layout()
        fig.savefig(f"../../images/grids/2d_fits_final_masked/{this_gal}_{col}.png", dpi = 300)
        plt.close(fig)

        print(f"{col} done")

    if os.path.exists("spiral_params/profiles_2d_final_masked.dat"):
        os.remove("spiral_params/profiles_2d_final_masked.dat")
    with open("spiral_params/profiles_2d_final_masked.dat", "w") as file:
...
for gal in gals:
    ...
    #    continue
    os.chdir(f"../{gal}")
    imfit_path = "masked/fit_masked.imfit"
    xc, yc, pa, ell = find_fit_params(imfit_path)
    col_arr, phi_arr, r_arr = read_shapes_file(pa, fname = "spiral_params/shapes_masked.dat")
