...
def fit_profile_2d(col_arr, phi_arr, r_arr, flip, d_cat):
    ...
    for i in range(len(col_arr)):
        ...
        axs[0, 0].imshow(arm_str, norm=LogNorm(vmin=0.001, vmax=0.3), cmap="inferno", origin="lower")
        ...
        for j in range(len(funcs)):
            ...
            try:
                ...
                axs[row, column].imshow(y_model, norm=LogNorm(vmin=0.001, vmax=0.3), cmap="inferno", origin="lower")
        ...
        if (arm_check in d_cat):
            ...
            if len(d_locs) == 1:
                ...
                if col == "green":
                    p0 = [*popt_sel, 10, d_locs[0], max_phi * 0.1, 10, d_locs[1], max_phi * 0.1]
                    p0[4] = max_phi * 0.1
                    p0[5] = max_phi * 0.1
                    p0[6] = max_phi * 0.1
                    p0[7] = max_phi * 0.9
            ...
            try:
                ...
                perr = np.sqrt(np.diag(pcov))
                y_model = func(X, *popt).reshape(np.shape(arm_str))
                axs[4, 2].imshow(y_model, norm=LogNorm(vmin=0.001, vmax=0.3), cmap="inferno", origin="lower")
        ...
        fig.tight_layout()
        fig.savefig(f"../../images/grids/uv/2d_fits_final/{this_gal}_{col}.png", dpi = 300)
        plt.close(fig)
