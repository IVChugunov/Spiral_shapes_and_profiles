...
def draw_grids(gal, col_fit, arms, phi_all, I_all, r_all, w_all, re_all, image, lines_good):
    ...
    for i in range(len(arms)):
        ...
        #print(gal)
        #print(arms[i])
        #print(np.nanmedian(r_err / wf))
        #print()

        #ax2.scatter(phi, r, c=arms[i], s = 3, alpha = 0.5)
        ax2.errorbar(phi, r, yerr=r_err, c=arms[i], marker = ".", markersize = 3, ls = "", elinewidth=0.5, alpha = 0.5)
        ax8.errorbar(phi, r, yerr=r_err, c=arms[i], marker = ".", markersize = 3, ls = "", elinewidth=0.5, alpha = 0.5)
        ...
        for j in range(len(fs)):
            ...
            try:
                ...
                resid = (fs[j](np.radians(phi - phi[0]), *popt) - r)
                #w_dev = np.nansum(resid / wf) / (len(r) - len(p))
                w_dev = np.sqrt(np.nanmean((resid ** 2) / (wf ** 2)))
                bic = (np.nansum(((fs[j](np.radians(phi - phi[0]), *popt) - r) ** 2) / (r_err ** 2)) / eff_psf_factor) + ((nparss[j] + 1) * np.log(len(r) / eff_psf_factor))
                bic = bic / len(r)
                if w_dev < 0.01:
                    wdevs[j] = np.nan
                else:
        ...
        wdevs_arr.append(f"{arms[i]} {name_sel} {phi[0]} {phi[-1]} {err_frac} {wdevs_str}")
        bics_arr.append(f"{arms[i]} {name_sel} {phi[0]} {phi[-1]} {len(r)} {bics_str}")

    with open("spiral_params/shapes.dat", "w") as file:
        for line in arm_shapes:
            file.write(f"{line}\n")

    with open("spiral_params/wdevs.dat", "w") as file:
        for line in wdevs_arr:
            file.write(f"{line}\n")

    with open("spiral_params/BICs.dat", "w") as file:
    ...
    fig.tight_layout()
    fig.savefig(f"../../images/grids/shapes/{gal}.png", dpi = 300)
