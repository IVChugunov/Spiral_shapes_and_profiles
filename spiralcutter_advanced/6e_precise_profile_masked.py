...
def fit_profiles_1d(col_arr, phi_arr, r_arr, flip):
    ...
    for i in range(len(col_arr)):
        ...
        #squeeze
        arm_str, sigma_str, phi_span, r = squeeze(arm_str_o, sigma_str_o, phi_span_o, r_o, 12)
        ...
        fig.tight_layout()
        fig.savefig(f"../../images/grids/precise_1d_fits_masked/{this_gal}_{col}.png", dpi = 300)
        plt.close(fig)

    if os.path.exists("spiral_params/profiles_precise_masked.dat"):
        os.remove("spiral_params/profiles_precise_masked.dat")
    with open("spiral_params/profiles_precise_masked.dat", "w") as file:
