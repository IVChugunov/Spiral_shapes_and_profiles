...
for gal in gals:
    ...
    folder = "str_arms_azavg"
    unfold_to_polar(image, mask, folder, xc, yc, pa, ell, col_arr, phi_arr, r_arr)

    image = fits.getdata("image.fits")
    folder = "str_arms_orig"
    unfold_to_polar(image, mask, folder, xc, yc, pa, ell, col_arr, phi_arr, r_arr)

    image = fits.getdata("residuals_nosp.fits")
    folder = "str_arms_residual"
    unfold_to_polar(image, mask, folder, xc, yc, pa, ell, col_arr, phi_arr, r_arr)

    image = fits.getdata("sigma.fits")
    folder = "str_arms_sigma"
    unfold_to_polar(image, mask, folder, xc, yc, pa, ell, col_arr, phi_arr, r_arr)

    image = fits.getdata("spirals_total.fits")
    folder = "str_arms_total"
    unfold_to_polar(image, mask, folder, xc, yc, pa, ell, col_arr, phi_arr, r_arr)
