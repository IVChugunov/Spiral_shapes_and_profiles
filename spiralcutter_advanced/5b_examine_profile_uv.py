...
def fit_profiles_1d(col_arr, phi_arr, r_arr, flip, d_cat):
    ...
    for i in range(len(col_arr)):
        ...
        w_e = np.where(w_e < np.nanmedian(w_e), np.nanmedian(w_e), w_e)
        ...
        for xi in [0, 1]:
            ...
            try:
                ...
                popt_i = p0
                pcov_i = np.zeros((len(p0), len(p0))) * np.nan
        ...
        axs[0, 0].imshow(arm_str_o, norm=LogNorm(vmin=0.001,vmax=0.3), cmap="inferno", origin="lower")
        ...
        fig.tight_layout()
        fig.savefig(f"../../images/grids/uv/1d_fits/{this_gal}_{col}.png", dpi = 300)
        plt.close(fig)

    os.makedirs("spiral_params", exist_ok = True)
...
def fit_profile_2d(col_arr, phi_arr, r_arr, flip, d_cat):
    ...
    for i in range(len(col_arr)):
        col = col_arr[i]
        fn = f"../../images/grids/uv/2d_fits/{this_gal}_{col}.png"
        ...
        axs[0, 0].imshow(arm_str, norm=LogNorm(vmin=0.001, vmax=1), cmap="inferno", origin="lower")
        axs[0, 1].set_visible(False)
        ...
        try:
            ...
            axs[1, 0].imshow(y_model, norm=LogNorm(vmin=0.001, vmax=1), cmap="inferno", origin="lower")
        ...
        try:
            ...
            axs[2, 0].imshow(y_model_r1r, norm=LogNorm(vmin=0.001, vmax=1), cmap="inferno", origin="lower")
        ...
        try:
            ...
            axs[0, 2].imshow(y_model_r2r, norm=LogNorm(vmin=0.001, vmax=1), cmap="inferno", origin="lower")
        ...
        if arm_check in d_cat:
            ...
            try:
                ...
                axs[1, 2].imshow(y_model_r2rd, norm=LogNorm(vmin=0.001, vmax=1), cmap="inferno", origin="lower")
        ...
        try:
            ...
            axs[2, 2].imshow(y_model_f2, norm=LogNorm(vmin=0.001, vmax=1), cmap="inferno", origin="lower")
        ...
        fig.tight_layout()
        fig.savefig(f"../../images/grids/uv/2d_fits/{this_gal}_{col}.png", dpi = 300)
        plt.close(fig)
